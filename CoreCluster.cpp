...
CoreCluster::CoreCluster(int nb_cores):nb_cores(nb_cores) {
    for(int i=0; i<nb_cores; i++){
        cores.emplace_back(i, this->main_bus, this->response_bus);
    }
}

CoreCluster::CoreCluster(int nb_cores, string program_name) {
    for(int i=0; i<nb_cores; i++){
